...
def main():
    ...
    # Initialize environment with human rendering for better visualization
    ...
    # Try to make the render window larger by modifying the environment
    if hasattr(env, 'screen_width'):
        env.screen_width = 1200  # Make wider
    if hasattr(env, 'screen_height'):
        env.screen_height = 800  # Make taller
    ...
    model_results = run_model_episode(model, device, env, initial_state, context_length=50)
